...
from app.intelligence import activate_autonomous_intelligence, agent_state, build_operator_briefing, intelligence_status, run_autonomous_cycle
...
async def subscribe(request: Request):
    ...
    result = _subscribe_user(
        ...
        voice_mode=data.get("voice_mode", "twin"),
    )
    return HTMLResponse(
        f"""
        <!doctype html>
...
def users_create(user: UserCreate) -> dict:
    try:
        return create_user(
            ...
            voice_mode=normalize_voice_mode(user.voice_mode),
        )
...
async def sms_webhook(request: Request) -> dict:
    """Twilio inbound SMS webhook for conversational market monitoring replies."""
    content_type = request.headers.get("content-type", "")
    raw_body = await request.body()
    ...
    body = data.get("Body", "")
    response = build_sms_reply(phone_number, body)
    user = response.get("user") or (find_user_by_phone(phone_number) if phone_number else None)
    reply_sent = False
    if phone_number and user:
        reply_user = {**user, "phone_number": phone_number, "sms_enabled": True}
        reply_sent = send_sms_alert(reply_user, response["reply"])

    return {
        "message": "SmokeSignal AI reply generated.",
        "action": response["action"],
        "reply": response["reply"],
        "reply_sms_sent": reply_sent,
