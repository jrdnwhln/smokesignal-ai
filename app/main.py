...
from app.ai_writer import generate_alert_text, normalize_voice_mode
...
from app.intelligence import (
    activate_autonomous_intelligence,
    agent_state,
    ...
    <html lang="en">
      <head>
        ...
        <style>
          ...
          .logo { display: block; width: min(360px, 82vw); height: auto; margin: 0 auto 20px; border-radius: 8px; }
          h1 { font-size: 30px; margin-bottom: 8px; text-align: center; }
          p { color: #cbd3dc; line-height: 1.5; text-align: center; }
          form { display: grid; gap: 14px; margin-top: 28px; }
          label { font-weight: 700; }
          input, select, button { width: 100%; box-sizing: border-box; border-radius: 6px; border: 1px solid #39424e; padding: 12px; font-size: 16px; }
          input, select { background: #161d24; color: #f5f7fa; }
          button { border: 0; background: #ff6b35; color: #111; font-weight: 800; cursor: pointer; }
          .notice { font-size: 13px; color: #aeb8c4; }
        </style>
      </head>
      <body>
        <main>
          ...
          <form method="post" action="/subscribe">
            ...
            <select id="voice_mode" name="voice_mode">
              ...
              <option value="normal_clanka">normal clanka</option>
            </select>
            <p class="notice">By subscribing, you agree to receive recurring hourly SmokeSignal AI market monitoring texts. Message and data rates may apply. Reply STOP to opt out. Not financial advice. Market alerts only.</p>
...
def _subscribe_user(name: str, phone_number: str, voice_mode: str) -> dict:
    user = upsert_user(name=name, phone_number=phone_number, sms_enabled=True, voice_mode=normalize_voice_mode(voice_mode))
    ...
    return {"message": "Subscription active.", "welcome_sms_sent": welcome_sent, "user": user, "watchlist": get_user_watchlist(user["id"])}
...
async def subscribe(request: Request):
    ...
    if "application/json" in content_type and raw_body:
        ...
        return _subscribe_user(payload.name, payload.phone_number, payload.voice_mode)

    data = _decode_request_data(raw_body, content_type)
    result = _subscribe_user(data.get("name", "SmokeSignal User"), data.get("phone_number", ""), data.get("voice_mode", "twin"))
    return HTMLResponse(
        f"""
        <!doctype html><html lang="en"><head><title>SmokeSignal AI Subscription</title></head>
...
@app.get("/scan/{symbol}")
def scan_symbol(symbol: str, voice_mode: str = "normal_clanka") -> dict:
...
def intelligence_status_route() -> dict:
    return intelligence_status()


@app.get("/intelligence/briefing")
def intelligence_briefing(voice_mode: str = "normal_clanka") -> dict:
    return build_operator_briefing(voice_mode)
...
def intelligence_cycle(voice_mode: str = "twin") -> dict:
    if settings.APP_ENV != "development":
        raise HTTPException(status_code=403, detail="Manual intelligence cycles only work in development.")
    return run_autonomous_cycle(voice_mode)


@app.post("/intelligence/activate")
def intelligence_activate(interval_minutes: int = 5, voice_mode: str = "twin") -> dict:
    if settings.APP_ENV != "development":
        raise HTTPException(status_code=403, detail="Manual intelligence activation only works in development.")
    return activate_autonomous_intelligence(interval_minutes=interval_minutes, voice_mode=voice_mode)


@app.post("/users")
def users_create(user: UserCreate) -> dict:
    try:
        return create_user(user.name, user.phone_number, user.sms_enabled, normalize_voice_mode(user.voice_mode))
...
def test_sms(request: TestSmsRequest) -> dict:
    ...
    sent = send_sms_alert({"phone_number": request.phone_number, "sms_enabled": True}, request.message)
    return {"sent": sent, "message": "SMS attempted only if SMS_ENABLED and Twilio settings are configured."}
...
async def sms_webhook(request: Request) -> dict:
    ...
    body = data.get("Body", "")
    if (body or "").strip().upper() in {"STOP", "UNSUBSCRIBE", "CANCEL", "QUIT"}:
        if phone_number:
    ...
    return {"message": "SmokeSignal AI webhook received.", "known_user": bool(user)}
